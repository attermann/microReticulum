...
RNS::Bytes serve_index(
	...
	VERBOSEF("Serving %s to link <%s> with identity <%s>", path.toString().c_str(), link_id.toHex().c_str(), (remote_identity ? remote_identity.hash().toHex().c_str() : RNS::Bytes{}.toHex().c_str()));
