#!/usr/bin/env bash
# Driver for the link lifecycle interop test (two-pass teardown).
...
RECV="$REPO_ROOT/test_interop/python_link_receiver.py"
SENDER="$REPO_ROOT/test_interop/link_interop_sender/.pio/build/native17/program"

if [[ ! -x "$SENDER" ]]; then
  echo "Sender binary not found at $SENDER. Build with:" >&2
  echo "  cd test_interop/link_interop_sender && pio run -e native17" >&2
  exit 1
fi

TMPDIR="$(mktemp -d -t microreticulum_link.XXXXXX)"
...
WAIT_DEADLINE=$(( $(date +%s) + TIMEOUT_S + 15 ))
while true; do
