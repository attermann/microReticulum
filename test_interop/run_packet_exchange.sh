#!/usr/bin/env bash
# Driver for the basic-packet-exchange interop test.
# Launches python_packet_receiver.py and the C++ packet_interop_sender
# binary, captures stdout/stderr, reports PASS iff both report SUCCESS.
...
RECV="$REPO_ROOT/test_interop/python_packet_receiver.py"
SENDER="$REPO_ROOT/test_interop/packet_interop_sender/.pio/build/native17/program"

if [[ ! -x "$SENDER" ]]; then
  echo "Sender binary not found at $SENDER. Build with:" >&2
  echo "  cd test_interop/packet_interop_sender && pio run -e native17" >&2
  exit 1
fi

TMPDIR="$(mktemp -d -t microreticulum_pkt.XXXXXX)"
